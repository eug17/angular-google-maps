...
export class GoogleMapsAPIWrapper {
  ...
  createInfoWindow(
    options?: mapTypes.InfoWindowOptions
  ): Promise<mapTypes.InfoWindow> {
  ...
  attachDrawingManager(
    controlPosition: mapTypes.ControlPosition = 9,
    drawingModes: mapTypes.DrawingModes = ["polygon"],
    polygonOptions: mapTypes.PolygonOptions,
    circleOptions?: mapTypes.DrawingCircleOptions,
  ...
  attachPolygonListeners<T>(eventName: string): Observable<T> {
    return Observable.create((observer: Observer<any>) => {
      this._drawingManager.addListener(eventName, (polygon: any) =>
        this._zone.run(() => observer.next(polygon))
      );
    });
  }

  updateDrawingManagerOptions(
    drawingModes: mapTypes.DrawingModes = [],
    controlPosition: mapTypes.ControlPosition = 9
  ) {
  ...
  fitBounds(
    latLng: mapTypes.LatLngBounds | mapTypes.LatLngBoundsLiteral
  ): Promise<void> {
  ...
  addExtraControll(control: mapTypes.ExtraControl) {
    return this._map.then(map => {
      ...
      let position =
        (control.position as keyof typeof mapTypes.ControlPosition) ||
        "TOP_CENTER";

      const controllPosition = map.controls[
